...
Было время когда программы писались в процедурном стиле, что конечно же имело свои преимущества, 
...
как посчитать время работы метода не нарушив при этом SRP ни в самом методе, ни в классе, где он вызывается? 
...
Создадим новый пакет и класс с именем соответствующим его контексту. Для того чтобы класс стал аспектным поставим аннотацию `@Aspect` и `@Component`.

Следующее понятие с которым нам необходимо познакомиться это **Advice**.
...
Такие методы делятся на несколько видов:
- **Before** (`@Before`)
...
- **modifiers-pattern** - Видимость метода (public, protected, private, *) 
- **ret-type-pattern** - Возвращаемы тип метода
- **declaring-type-pattern** - Пакет или класс (например: com.app.service.* — применяется ко всем классам в этом пакете, com.app.service.UserService — применяется только к классу UserService, * — все)
- **name-pattern** - имя метода (пример: set* - all setters)
...
- **throws-pattern** - метод, выбрасывающий это исключение.
